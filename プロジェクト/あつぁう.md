...
setDateBtn.onclick = async () => {
    new Notice("DEBUG: Button Clicked");
    const api = getQuickAdd();
    if (!api) { new Notice("DEBUG: QuickAdd API not found"); return; }
    
    const activeFile = app.workspace.getActiveFile();
    if (!activeFile) { new Notice("DEBUG: No active file"); return; }
    new Notice(`DEBUG: Active File: ${activeFile.basename}`);
    ...
    new Notice(`DEBUG: Found ${taskLines.length} tasks`);
    ...
    if (!selectedTasks || selectedTasks.length === 0) { new Notice("DEBUG: No tasks selected"); return; }
    ...
    if (!dateObj.isValid()) { new Notice("無効な日付"); return; }
    const dateStr = dateObj.format("YYYY-MM-DD");
    new Notice(`DEBUG: Target Date: ${dateStr}`);
    ...
    await app.vault.modify(activeFile, newContent);
    new Notice("DEBUG: Project file updated");
    ...
    for (const item of tasksToRemove) {
        ...
        if (oldFile) {
            ...
            if (oldContent.includes(item.line)) {
                ...
                new Notice(`DEBUG: Removed from ${item.date}`);
            } else {
                new Notice(`DEBUG: Task not found in ${item.date}`);
            }
        } else {
            new Notice(`DEBUG: Schedule file not found: ${item.date}`);
        }
    }
    ...
    if (addedCount > 0) {
        await app.vault.modify(scheduleFile, scheduleContent);
        new Notice(`DEBUG: Added to ${dateStr}`);
    }
...
### あｓｄｆｊ
- [ ] テスト #デスクワーク ⏱️ 30 📅 2025-12-05 🔗 あつぁう
## メモ
