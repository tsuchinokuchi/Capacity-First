...
addButton.onclick = async () => {
  try {
    ...
    // サブタスクカテゴリを選択（既存のサブタスクカテゴリを取得）
    let projectContent = await app.vault.read(activeFile);
    const subtaskMatches = projectContent.matchAll(/^### (.+)$/gm);
    const existingSubtasks = Array.from(subtaskMatches).map(m => m[1]).filter(s => s !== '概要' && s !== '期間' && s !== '進捗' && s !== 'サブタスク（1階層まで）' && s !== 'メモ');
    ...
    // プロジェクト名を取得（ファイル名から拡張子を除く）
    const projectName = activeFile.basename;
    
    // タスク行を作成（プロジェクト情報をメタデータとして追加）
...
### 資料作成
- [ ] パワポ作成 #デスクワーク ⏱️ 120 📅 2025-11-09
- [ ] パワポ作成 #デスクワーク ⏱️ 120 📅 2025-11-10
- [x] aaa #デスクワーク ⏱️ 60 📅 2025-11-06 ✅ 2025-11-06
- [x] aaaaaaa #デスクワーク ⏱️ 60 📅 2025-11-06 🔗 見積もり管理
- [x] aaaa #デスクワーク ⏱️ 15 📅 2025-11-16 ⏰ 2025-11-06 🔗 見積もり管理 ✅ 2025-11-08

### 打ち合わせ
- [ ] 岡本さんとMT #顧客対応 ⏱️ 60 📅 2025-11-12
