...
async function main() {
  try {
    // 1. プロジェクト名を入力
    const projectName = await QuickAdd.quickAddApi.inputPrompt(
      "プロジェクト名を入力してください"
    );
    ...
    // 2. 概要を入力（オプション）
    const description = await QuickAdd.quickAddApi.inputPrompt(
    ...
    const parseDate = (input) => {
      ...
      // Normalize: Full-width to half-width, replace separators
      const normalized = input.trim()
        .replace(/[０-９]/g, s => String.fromCharCode(s.charCodeAt(0) - 0xFEE0))
        .replace(/[－／．]/g, s => s === '－' ? '-' : s === '／' ? '/' : '.');

      // 1. YYYY-MM-DD (slash/dot/hyphen)
      if (/^\d{4}[-./]\d{1,2}[-./]\d{1,2}$/.test(normalized)) {
        const d = moment(normalized.replace(/[./]/g, '-'), "YYYY-MM-DD");
        return d.isValid() ? d : null;
      }

      // 2. MM-DD (slash/dot/hyphen) -> Append Current Year
      if (/^\d{1,2}[-./]\d{1,2}$/.test(normalized)) {
        const currentYear = moment().year();
        const d = moment(`${currentYear}-${normalized.replace(/[./]/g, '-')}`, "YYYY-MM-DD");
        return d.isValid() ? d : null;
      }
    ...
    const startDateInput = await QuickAdd.quickAddApi.inputPrompt(
    ...
    const endDateInput = await QuickAdd.quickAddApi.inputPrompt(
    ...
    if (existingFile) {
      const shouldOverwrite = await QuickAdd.quickAddApi.yesNoPrompt(
