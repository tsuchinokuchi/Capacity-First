...
## 0. 共有プリセットを適用する
1. QuickAddを有効化し、**Enable API** を ON。
2. `.obsidian/plugins/quickadd/data.json` をバックアップ。
3. `config/quickadd_preset.json` を `.obsidian/plugins/quickadd/data.json` にコピーして上書き。
4. Obsidianを再起動し、`Ctrl+P` → `QuickAdd: タスク追加` が表示されれば完了。

## 1. プリセットに含まれるChoice

| QuickAdd Choice | スクリプト | 主な用途 |
| --- | --- | --- |
| タスク追加 | `scripts/タスク追加.js` | 容量チェック付きタスク登録 |
| タスク移動 | `scripts/タスク移動.js` | 次の出勤日へタスクを移動 |
| タスクプールからスケジュールに移動 | `QuickAdd Macros/タスクプールからスケジュールに移動.js` | タスクプール→日次スケジュール |
| 繰り返しタスク展開 | `scripts/繰り返しタスク展開.js` | 定義済み繰り返しタスクの展開 |
| 容量チェック | `scripts/容量チェック.js` | スケジュールテンプレのボタン経由で使用 |
| タスク完了 / タスク削除 | `scripts/タスク完了.js` / `scripts/タスク削除.js` | タスクの後処理 |
| プロジェクト作成 / サブタスク追加 | `scripts/プロジェクト作成.js` / `scripts/サブタスク追加.js` | プロジェクト管理 |
